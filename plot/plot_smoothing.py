...
def plot_original_smoothing (Path, inVector, originalCSV, smoothingCSV) : #, originalCSV_p, smoothingCSV_p):
    """
...
#    print (o_c_join)
    smoothing_complete = pd.read_csv(smoothingCSV)
...
#    print (times)
    ...
    with PdfPages(os.path.join(Path,'Plots_Hants_PRScor_hants.pdf'),'a') as pdf:
...
#            values_s_p = []
    ...
    #            print (Date.strftime('%Y%m%d'))
                ...
                values_s_c.append(s_c_join[Date.strftime('%Y%m%d')+'_HMean'][index])
...
if __name__=="__main__":
    ...
    plot_original_smoothing (Path, inVector, originalCSV, hantsCSV, whitCSV)